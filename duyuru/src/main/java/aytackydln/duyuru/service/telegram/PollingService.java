...
public class PollingService implements DisposableBean {
	...
	public void apologyInvalidMessages() {
		final Set<UserEntity> apologyList = new HashSet<>();
		final Updates invalidUpdates = getUpdates();
		for (final Update invalidUpdate : invalidUpdates) {
			final UserEntity userEntity = new UserEntity(); //TODO refactor
			final TelegramMessage message = invalidUpdate.getMessage();
			if (message != null)
				userEntity.setId(message.getChat().getPlatformChatId());
			else continue;
			apologyList.add(userEntity);
			lastUpdate = invalidUpdate.getUpdateId();
		}
		lastUpdate++;

		for (final UserEntity userEntity : apologyList) {
			telegramService.sendMessage(userEntity,
					dictionaryKeeper.getTranslation(userEntity.getLanguage(), "UNAVAILABLE_APOLOGY"));
		}
	}
